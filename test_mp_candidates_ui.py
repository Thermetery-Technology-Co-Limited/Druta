...
class CandidateUi(unittest.TestCase):
    ...
    def test_selection_and_scan_refuse_while_bus_or_profile_owned(self):
        for field, value in (("_i2c_busy", True), ("_profile_pending", ("p",)),
            ...
            with self.subTest(field=field), patch("druta.dpg.does_item_exist", return_value=True), \
                    patch("druta.dpg.set_value") as set_value, \
    ...
    def test_disappearing_controller_clears_a_previously_valid_pass(self):
        ...
        with patch("druta.dpg.does_item_exist", return_value=True), \
                patch("druta.dpg.get_value", return_value=True):
    ...
    def test_refresh_rebuilds_only_regulator_controls(self):
        ...
        with patch("druta.dpg.does_item_exist", return_value=True), \
                patch("druta.dpg.delete_item") as delete, \
                patch("druta.dpg.group", side_effect=lambda **kw: nullcontext()):
