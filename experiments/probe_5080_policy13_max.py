"""Probe declared policy-13 limits ONLY while the GPU is idle.

Targets are fixed: 305 A, declared 5001 A, and 5001.001 A. Each trial restores
the full original selected record in finally, before any report I/O. Values
above 300 A are API-acceptance tests, not operating-current recommendations.
"""
import argparse
import ctypes as C
import hashlib
...
TARGETS = (305000, 5001000, 5001001)
OUT = ROOT / "experiments/power-5080-20260908/policy13-maximum"
...
def snapshot(gpu):
    ...
    state = n.u32()
    rc = gpu.nvml.dll.nvmlDeviceGetPerformanceState(gpu.nvml.dev, C.byref(state))
    assert rc == 0
    return {"time": time.time(), "pstate": state.value, "telemetry": telemetry,
            "policy_13": sample["policies"][13], "policy_14": sample["policies"][14],
            "board_policy": sample["policies"][8]}


def require_idle(sample):
    assert sample["pstate"] == 8, "GPU is not in idle P8"
    assert sample["telemetry"]["power_w"] < 50, "GPU board power not idle"
    assert sample["policy_13"]["value"] < 30000, "Core current not idle"


def send(gpu, transport, original, target):
    assert target in TARGETS + (300000,)
    assert len(original) * 4 == PARAM_BYTES
    assert original[RECORD_WORD] == 0x12 and original[LIMIT_WORD] == 300000
    params = list(original)
    params[4], params[LIMIT_WORD] = 0x2000, target
...
def main():
    ...
    first = snapshot(gpu)
    meta = first["policy_13"]
    assert (meta["minimum"], meta["default"], meta["maximum"], meta["limit"]) == (1, 300000, 5001000, 300000)
    ...
    for target in TARGETS:
        ...
        try:
            ...
            if attempted:
                trial["restore"] = send(gpu, transport, original, 300000)
            ...
            trial["effective_limit_restored"] = trial["restored"]["policy_13"]["limit"] == 300000
            save_report(path, report)
        ...
        print(json.dumps({"target_ma": target, "ntstatus": trial["set"]["ntstatus"],
                          ...
                          "effective_limits_ma": sorted({s["policy_13"]["limit"] for s in trial["active"]}),
