...
git -C build\vista\dearpygui apply ..\..\..\src\druta\tools\vista\dearpygui-vista.patch
...
`ucrt-10240.json` and collected by `src/druta/tools/collect_vista_redist.py`.
...
```bat
python src\druta\tools\smoke_vista_dialog.py --output dialog-hardware.json
python src\druta\tools\smoke_vista_dialog.py --output dialog-warp.json --warp
```
