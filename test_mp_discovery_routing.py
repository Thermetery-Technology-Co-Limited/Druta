...
class DiscoveryRouting(unittest.TestCase):
    def setUp(self):
        ...
        self.scanner_patch = patch("mp2888.discover", return_value=[self.hit])
        self.scan = self.scanner_patch.start()
        self.addCleanup(self.scanner_patch.stop)
        self.profiles_patch = patch("railctl.load_profiles", return_value=[self.mp, self.generic])
    ...
    def test_kepler_ncp_and_mp_candidates_are_both_returned(self):
        ...
        with patch("ncp4206.NCP4206", return_value=ncp), \
                patch("ncp4206.DISCOVERY_PORTS", (2,)):
